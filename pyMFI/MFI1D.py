...
def load_HILLS(hills_name="HILLS"):
    ...
    for file in glob.glob(hills_name):
        ...
        hills = hills[
            :-1
        ]  # Last hill is removed from output array, as no complete set of COLVAR data is available of that hill.
        hills0 = np.array(hills[0])
        hills0[3] = 0
        hills = np.concatenate(
            ([hills0], hills)
        )  # One dummy hills with zero height is introduced at the beginning of the output array, so that the forces of the first few positions are calculated without the effect of any bias hill.
    return hills
...
def load_position(position_name="position"):
    ...
    return colvar[
        :-1, 1
    ]  # last position can be removed, since it is not part of a complete set of positions in a window of constant bias.
...
def find_periodic_point(x_coord, min_grid, max_grid, periodic, grid_ext):
    ...
    if periodic == 1:
        ...
        if x_coord < min_grid + grid_ext:
            coord_list.append(x_coord + grid_length)
...
def window_forces(
    ...
    for j in range(len(periodic_hills)):
        ...
        Fbias_window += (
            height_meta
            / sigma_meta2
            * np.multiply(kernelmeta, (grid - periodic_hills[j]))
        )
...
def find_hp_force(hp_centre, hp_kappa, grid, min_grid, max_grid, grid_space, periodic):
    ...
    if periodic == 1:
        ...
        if hp_centre < grid_centre:
            ...
            F_harmonic[index_period:] = hp_kappa * (
                grid[index_period:] - hp_centre - grid_length
            )
...
def find_lw_force(lw_centre, lw_kappa, grid, min_grid, max_grid, grid_space, periodic):
    ...
    if periodic == 1:
        ...
        if lw_centre < grid_centre:
            ...
            F_harmonic[index_period:] = (
                2 * lw_kappa * (grid[index_period:] - lw_centre - grid_length)
            )
...
def find_uw_force(uw_centre, uw_kappa, grid, min_grid, max_grid, grid_space, periodic):
    ...
    if periodic == 1:
        ...
        if uw_centre < grid_centre:
            ...
            index_period = index(uw_centre - grid_length / 2, min_grid, grid_space)
            F_harmonic[:index_period] = (
                2 * uw_kappa * (grid[:index_period] - uw_centre + grid_length)
            )
...
def intg_1D(Force, dx):
    ...
    for j in range(len(Force)):
        ...
        if N % 2 == 0:
            fes[j] = dx / 6 * (
                np.sum(y[: N - 3 : 2] + 4 * y[1 : N - 3 + 1 : 2] + y[2 : N - 3 + 2 : 2])
                + np.sum(y[1 : N - 2 : 2] + 4 * y[1 + 1 : N - 1 : 2] + y[1 + 2 : N : 2])
            ) + dx / 4 * (y[1] + y[0] + y[-1] + y[-2])
        else:
            fes[j] = dx / 3.0 * np.sum(y[: N - 2 : 2] + 4 * y[1 : N - 1 : 2] + y[2:N:2])
    fes = fes - min(fes)
    return fes
...
def MFI_1D(
    ...
    save_intermediate_fes_error_cutoff=False,
...
def get_cutoff_1D(Ftot_den=None, Ftot_den_cutoff=0.1, FES=None, FES_cutoff=-1):
    ...
    if Ftot_den != None:
        nbins = len(Ftot_den)
    elif FES != None and FES_cutoff > 0:
        nbins = len(FES)
    else:
        print(
    ...
    if Ftot_den != None:
        cutoff = np.where(Ftot_den > Ftot_den_cutoff, 1.0, 0.0)
    if FES != None and FES_cutoff > 0:
...
@njit
def patch_forces_ofe(
    force_vector,
    PD_limit=1e-10,
    use_weighted_st_dev=True,
    ofe_progression=True,
    Ftot_den_cutoff=-1,
    non_exploration_penalty=0,
):
    ...
    if ofe_progression == False:
        ...
        if use_weighted_st_dev == True:
            PD_ratio = np.where(PD_diff > 0, PD_patch2 / PD_diff, 0)
        else:
            PD_ratio = np.where(PD_diff > 0, PD2_patch / PD_diff, 0)
        OFV = np.multiply(
            np.where(PD_patch > 0, OFV_num_patch / PD_patch, 0) - np.square(F_patch),
            PD_ratio,
        )
        if Ftot_den_cutoff > 0:
            OFV *= get_cutoff_1D(Ftot_den=PD_patch, Ftot_den_cutoff=Ftot_den_cutoff)
        if non_exploration_penalty > 0:
            OFV = np.where(PD_patch > Ftot_den_cutoff, OFV, non_exploration_penalty**2)
    ...
    if ofe_progression == True:
        ...
        for i in range(len(force_vector)):
            ...
            if use_weighted_st_dev == True:
                ...
                PD_ratio = np.where(PD_diff > 0, PD2_patch / PD_diff, 0)
            OFV = np.multiply(
                np.where(PD_patch > 0, OFV_num_patch / PD_patch, 0) - np.square(F),
                PD_ratio,
            )
            if Ftot_den_cutoff > 0:
                OFV *= get_cutoff_1D(Ftot_den=PD_patch, Ftot_den_cutoff=Ftot_den_cutoff)
            if non_exploration_penalty > 0:
                OFV = np.where(
                    PD_patch > Ftot_den_cutoff, OFV, non_exploration_penalty**2
                )
            OFE = np.where(OFV > 1e-5, np.sqrt(OFV), 0)
            AOFE = np.sum(OFE) / np.count_nonzero(OFE)
...
@njit
def patch_forces_ofe_AD(
    force_vector,
    grid,
    y,
    PD_limit=1e-10,
    use_weighted_st_dev=True,
    error_progression=True,
    Ftot_den_cutoff=-1,
    FES_cutoff=-1,
    non_exploration_penalty=0,
    set_fes_minima=None,
    ...
    if error_progression == False:
        ...
        if set_fes_minima == "first":
            FES = FES - FES[0]
        if Ftot_den_cutoff > 0 or FES_cutoff > 0:
            cutoff = get_cutoff_1D(
                Ftot_den=PD_patch,
        ...
        volume_progression = np.ones_like(Aofe_progression) * (
            np.count_nonzero(cutoff) / nbins
        )
...
@njit
def bootstrap_forw_back(
    grid, forward_force, backward_force, n_bootstrap, set_fes_minima=None
):
    ...
    for iteration in range(n_bootstrap):
        ...
        if (iteration + 1) % 50 == 0:
            print(
                "Iteration:", iteration + 1, "- sd:", round(sd_fes_prog[iteration], 5)
            )
...
def plot_recap(
    ...
    error_log_scale=1,
    cv="CV",
    time_axis_name="simulation time",
):
    ...
    if y_ref is not None:
        axs[0, 0].plot(X, y_ref, label="Reference", color="red", alpha=0.3)
...
def zero_to_nan(input_array):
    ...
    for ii in range(len(input_array)):
        if input_array[ii] == 0:
            output_array[ii] = np.nan
        else:
            output_array[ii] = input_array[ii]
    return output_array
