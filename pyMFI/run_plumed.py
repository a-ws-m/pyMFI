...
def run_langevin1D(
    simulation_steps,
    analytical_function="7*x^4-23*x^2",
    periodic="NO",
    ...
    if periodic == "YES":
        ...
        print(
            'Please specify if you want to use periodic boundaries using "YES" or "NO". \nFor now, no periodic boundaries are assumed'
        )
        periodic_boundaries = "NO"
...
nstep {}
ipos {}""".format(
                temperature, time_step, simulation_steps, initial_position
            ),
            file=f,
        )
...
bb: BIASVALUE ARG=ff""".format(
                analytical_function, periodic_boundaries
            ),
            file=f,
        )
    ...
    with open("plumed.dat", "a") as f:
        ...
        if gaus_height > 0:
            f.write(
                "METAD ARG=p.x SIGMA={} HEIGHT={} BIASFACTOR={} GRID_MIN={} GRID_MAX={} GRID_BIN={} PACE={} \
TEMP={} \n".format(
                    gaus_width,
                    gaus_height,
                    biasfactor,
        ...
        if lw_kappa > 0:
            f.write(
                "LOWER_WALLS ARG=p.x AT={} KAPPA={} LABEL=lowerwall \n".format(
                    lw_centre, lw_kappa
                )
            )
        ...
        if uw_kappa > 0:
            f.write(
                "UPPER_WALLS ARG=p.x AT={} KAPPA={} LABEL=upperwall \n".format(
                    uw_centre, uw_kappa
                )
            )
...
def run_langevin2D(
    simulation_steps,
    analytical_function="7*x^4-23*x^2+7*y^4-23*y^2",
    periodic_f="NO",
...
bb: BIASVALUE ARG=ff""".format(
                periodic_f, analytical_function
            ),
            file=f,
        )
    ...
    with open("plumed.dat", "a") as f:
        ...
        if gaus_height > 0:
            f.write(
                "METAD ARG=p.x,p.y SIGMA={},{} HEIGHT={} BIASFACTOR={} GRID_MIN={},{} GRID_MAX={},{} GRID_BIN={},{} PACE={} \
TEMP={} FILE=HILLS{}\n".format(
                    gaus_width_x,
                    gaus_width_y,
                    gaus_height,
        ...
        if hp_kappa_x > 0 or hp_kappa_y > 0:
            f.write(
                "RESTRAINT ARG=p.x,p.y AT={},{} KAPPA={},{} LABEL=restraint \n".format(
                    hp_centre_x, hp_centre_y, hp_kappa_x, hp_kappa_y
                )
            )
        ...
        if lw_kappa_x > 0 or lw_kappa_y > 0:
            f.write(
                "LOWER_WALLS ARG=p.x,p.y AT={},{} KAPPA={},{} LABEL=lowerwall \n".format(
                    lw_centre_x, lw_centre_y, lw_kappa_x, lw_kappa_y
                )
            )
        ...
        if uw_kappa_x > 0 or uw_kappa_y > 0:
            f.write(
                "UPPER_WALLS ARG=p.x,p.y AT={},{} KAPPA={},{} LABEL=upperwall \n".format(
                    uw_centre_x, uw_centre_y, uw_kappa_x, uw_kappa_y
                )
            )
        ...
        if position_pace == 0:
            position_pace = int(gaus_pace / 10)
        f.write(
            "PRINT FILE=position{} ARG=p.x,p.y STRIDE={}".format(
                file_extension, position_pace
            )
        )
...
bb: BIASVALUE ARG=ff
METAD ARG=p.x,p.y PACE={} SIGMA={},{} HEIGHT={} GRID_MIN=-4,-4 GRID_MAX=4,4 GRID_BIN=400,400 BIASFACTOR={} TEMP=120 FILE=HILLSinve_{}""".format(
                gaus_pace, sigma, sigma, height, biasfactor, file_extension
            ),
            file=f,
        )
    ...
    with open("plumed.dat", "a") as f:
        ...
        if external_bias_file != "":
            f.write(
                "EXTERNAL ARG=p.x,p.y FILE={} LABEL=external \n".format(
                    external_bias_file
                )
            )
        ...
        if position_pace == 0:
            position_pace = int(gaus_pace / 10)
        f.write(
            "PRINT FILE=positioninve_{} ARG=p.x,p.y STRIDE={}".format(
                file_extension, position_pace
            )
        )
...
ipos {},{}
periodic false""".format(
                simulation_steps, initial_position_x, initial_position_y
            ),
            file=f,
        )
...
def find_alanine_dipeptide_input(
    initial_position_x=0.0, initial_position_y=0.0, file_extension=""
):
    ...
    start_region = [
        str(initial_position_x - 0.5),
        str(initial_position_x + 0.5),
        str(initial_position_y - 0.5),
        str(initial_position_y + 0.5),
    ]
    ...
    with open("plumed_traj.dat") as f:
        lines = f.readlines()
    lines[3] = (
        "UPDATE_IF ARG=phi,psi MORE_THAN="
        + start_region[0]
        + ","
        + start_region[2]
        + " LESS_THAN="
    ...
    os.system(
        "plumed driver --plumed plumed_traj.dat --mf_xtc 0traj_comp.xtc > /dev/null"
    )

    # Remove a random number of structures, to avoid using always the same structure.
    total_n_lines = int(
        os.popen("wc -l structure" + str(file_extension) + ".gro").read().split()[0]
    )
    del_structure_lines = int(25 * randint(1, (int(total_n_lines / 25) - 1)))
    os.system(
        "sed -i -e '1,"
        + str(del_structure_lines)
        + "d' structure"
        + str(file_extension)
        + ".gro"
    )
    ...
    find_input_structure = subprocess.Popen(
        [
            "gmx",
            "grompp",
            "-f",
            "gromppvac.mdp",
            "-c",
            "structure" + str(file_extension) + ".gro",
            "-p",
            "topology.top",
            "-o",
            "input" + str(file_extension) + ".tpr",
        ],
    ...
    output_find_input_structure, errors_find_input_structure = (
        find_input_structure.communicate()
    )
...
psi: TORSION ATOMS=@psi-2""",
    ...
    with open("plumed.dat", "a") as f:
        ...
        if gaus_height > 0:
            f.write(
                "METAD ARG=phi,psi SIGMA={},{} HEIGHT={} BIASFACTOR={} GRID_MIN={},{} GRID_MAX={},{} GRID_BIN={},{} PACE={} \
TEMP={} FILE=HILLS{}\n".format(
                    gaus_width_x,
                    gaus_width_y,
                    gaus_height,
        ...
        if hp_kappa_x > 0 or hp_kappa_y > 0:
            f.write(
                "RESTRAINT ARG=phi,psi AT={},{} KAPPA={},{} LABEL=restraint \n".format(
                    hp_centre_x, hp_centre_y, hp_kappa_x, hp_kappa_y
                )
            )
            if print_bias == 1:
        ...
        if lw_kappa_x > 0 or lw_kappa_y > 0:
            f.write(
                "LOWER_WALLS ARG=phi,psi AT={},{} KAPPA={},{} LABEL=lowerwall \n".format(
                    lw_centre_x, lw_centre_y, lw_kappa_x, lw_kappa_y
                )
            )
        ...
        if uw_kappa_x > 0 or uw_kappa_y > 0:
            f.write(
                "UPPER_WALLS ARG=phi,psi AT={},{} KAPPA={},{} LABEL=upperwall \n".format(
                    uw_centre_x, uw_centre_y, uw_kappa_x, uw_kappa_y
                )
            )
        ...
        if position_pace == 0:
            position_pace = int(gaus_pace / 10)
        f.write(
            "PRINT FILE=position{} ARG=phi,psi STRIDE={}".format(
                file_extension, position_pace
            )
        )
    ...
    find_input_file = subprocess.Popen(
        [
            "gmx",
            "mdrun",
            "-s",
...
def run_alanine_dipeptide_1D(
    ...
    with open("plumed.dat", "w") as f:
        print(
            """MOLINFO STRUCTURE=reference.pdb
{}: TORSION ATOMS=@{}-2""".format(
                tors_angle, tors_angle
            ),
            file=f,
        )
    ...
    with open("plumed.dat", "a") as f:
        ...
        if gaus_height > 0:
            f.write(
                "METAD ARG={} SIGMA={} HEIGHT={} BIASFACTOR={} GRID_MIN={} GRID_MAX={} GRID_BIN={} PACE={} \
TEMP={} FILE=HILLS{}\n".format(
                    tors_angle,
                    gaus_width,
                    gaus_height,
        ...
        if hp_kappa > 0:
            f.write(
                "RESTRAINT ARG={} AT={} KAPPA={} LABEL=restraint \n".format(
                    tors_angle, hp_centre, hp_kappa
                )
            )
            if print_bias == 1:
                f.write(
                    "PRINT FILE=restraint{} ARG={},restraint.bias,restraint.force2 STRIDE=100\n".format(
                        file_extension, tors_angle
                    )
                )
        ...
        if lw_kappa > 0:
            f.write(
                "LOWER_WALLS ARG={} AT={} KAPPA={} LABEL=lowerwall \n".format(
                    tors_angle, lw_centre, lw_kappa
                )
            )
            if print_bias == 1:
                f.write(
                    "PRINT FILE=lowerwall{} ARG={},lowerwall.bias,lowerwall.force2 STRIDE=100\n".format(
                        file_extension, tors_angle
                    )
                )
        ...
        if uw_kappa > 0:
            f.write(
                "UPPER_WALLS ARG={} AT={} KAPPA={} LABEL=upperwall \n".format(
                    tors_angle, uw_centre, uw_kappa
                )
            )
            if print_bias == 1:
                f.write(
                    "PRINT FILE=upperwall{} ARG={},upperwall.bias,upperwall.force2 STRIDE=100\n".format(
                        file_extension, tors_angle
                    )
                )
        ...
        if position_pace == 0:
            position_pace = int(gaus_pace / 10)
        f.write(
            "PRINT FILE=position{} ARG={} STRIDE={}".format(
                file_extension, tors_angle, position_pace
            )
        )
...
def run_langevin1D_plumed_fes(
    ...
    if grid_min_out == None:
        grid_min_out = grid_min_plumed
    if grid_max_out == None:
        grid_max_out = grid_max_plumed
    if grid_bin_out == None:
        grid_bin_out = grid_bin_plumed
...
PRINT FILE=position ARG=p.x STRIDE=10""".format(
                analytical_function,
                periodic_boundaries,
                gaus_width,
                gaus_height,
...
def make_external_bias_1D(
    grid_old,
    FES,
    Ftot,
    grid_min_plumed=None,
    grid_max_plumed=None,
    nbins_plumed=None,
    file_name_extension="",
    return_array=None,
):
    ...
    head_text = (
        "#! FIELDS p.x external.bias der_p.x\n#! SET min_p.x "
        + str(grid_min_plumed)
        + "\n#! SET max_p.x "
        + str(grid_max_plumed)
        + "\n#! SET nbins_p.x "
    ...
    np.savetxt(
        "external_bias.dat" + file_name_extension,
